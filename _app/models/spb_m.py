...
class SpbModel(db.Model):
    __tablename__ = 'spb'
    id = db.Column(db.String, primary_key=True, default=lambda: uuid.uuid4().hex)
    nama_spb = db.Column(db.String)
    pemilik_spb = db.Column(db.String)
    ...
    def __init__(self, nama_spb, pemilik_spb, bendera, kelas, tahun, grt, dwt, nrt, loa, sarat_muka, sarat_belakang, ket, add_by):
        self.nama_spb = nama_spb
        self.pemilik_spb = pemilik_spb
    ...
    def json(self):
        return {
            'id': self.id,
            'nama_spb': self.nama_spb,
            'pemilik_spb': self.pemilik_spb,
